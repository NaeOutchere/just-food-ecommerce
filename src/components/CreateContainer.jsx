...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on('state_changed', (snapshot) => {
      ...
      console.log(error);
      setFields(true);
      setMsg('🚨 Error while uploading: Please try again later.');
      ...
      getDownloadURL(uploadTask.snapshot.ref).then(downloadURL => {
        ...
        setIsLoading(false);
        setFields(true);
        setMsg('Item has been uploaded Successfully🎉');
  ...
  const saveDetails = () => {
    setIsLoading(true);
    try {
      if (!title || !calories || !imageAsset || !price || !qty || !category) {
        setFields(true);
        ...
        <div className="w-[90%] md:w-[50%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
          {
                ...
                <>
                  {!imageAsset ? ( <>
                    ...
                    <><div className='relative h-full'>
                      ...
                      <button type='button' className='absolute bottom-3 right-3 p-3 rounded-full bg-red-500 text-xl cursor-pointer outline-none hover:shadow-md duration-500 transition-all ease-in-out' onClick={deleteImage}> <MdDelete className='textwhite' /> </button>
                      </div>
                      </>
                  )}
                </>
            ...
            <div className='w-full flex flex-col md:flex-row items-center gap-3'>
              ...
              <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
                <MdOutlineAdd className='text-gray-700 text-2xl' />
                <input type="text"
                required
                value={qty}
            ...
            <div className='flex items-center w-full'>
              <button type='button' className='ml-0 md:ml-auto w-full md:w-auto border-none outline-none bg-emerald-500 px-12 py-2 rounded-lg text-lg text-white font-semibold' onClick={saveDetails}>Add New Item</button>
            </div>
        </div>
    </div>
  );
